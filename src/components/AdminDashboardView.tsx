...
export const AdminDashboardView: React.FC<AdminDashboardViewProps> = ({
  ...
  const [decreeForm, setDecreeForm] = useState({
    ...
    appointeePhotoUrl: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=300&q=80',
    thanksReason: 'تقديراً لعطائه المشهود ومساعيه المباركة في خدمة أبناء العمومة',
  ...
  // ==========================================
  const handleAutoGenerateDecreeText = (customName?: string, customPosition?: string, customBranch?: string, isAppt = decreeForm.isAppointment, isThanks = decreeForm.isThanksOrHonorary) => {
    ...
    if (isThanks) {
      const generatedTitle = 'قرار شكر وتقدير وعرفان ووسام وفاء';
      const generatedPreamble = `بناءً على الصلاحيات المخولة للأمانة العامة والهيئة العليا لتجمع السادة الأشراف بني هاشم بجمهورية مصر العربية، وتقديراً للجهود المباركة والعطاء المخلص والمساعي الحميدة في خدمة أبناء العمومة وترسيخ أواصر المودة والتكافل، قررنا ما هو آت:`;
      const generatedArticles = [
        `منح السيد الشريف / ${name || '[اسم المكرم]'} ${branch ? `(${branch})` : ''} وسام الشكر والتقدير وشهادة العرفان الهاشمية تقديراً لعطائه المشهود ومساهماته الجليلة.`,
        `يُسجل هذا التكريم في لوحة الشرف الرسمية للأمانة العامة بتجمع السادة الأشراف بني هاشم بجمهورية مصر العربية.`,
        `يُعمل بهذا القرار من تاريخ صدوره، ويُسلّم للمكرم ويُنشر باللوحة الرسمية للتجمع.`
      ];
  ...
  const handleSelectMemberForAppointee = (memberId: string) => {
    ...
    if (found) {
      ...
      handleAutoGenerateDecreeText(found.fullName, decreeForm.appointeePosition, found.branch, decreeForm.isAppointment, decreeForm.isThanksOrHonorary);
    }
  };
  ...
  const handlePreviewDecree = () => {
    const dec: OfficialDecree = {
      ...
      decreeType: decreeForm.isThanksOrHonorary ? 'honorary' : (decreeForm.isAppointment ? 'appointment' : decreeForm.decreeType),
      isAppointment: decreeForm.isAppointment,
      isThanksOrHonorary: decreeForm.isThanksOrHonorary,
      includePhoto: decreeForm.includePhoto,
      thanksReason: decreeForm.thanksReason,
      title: decreeForm.title || (decreeForm.isThanksOrHonorary ? 'قرار شكر وتقدير وعرفان' : (decreeForm.isAppointment ? 'قرار تعيين وتكليف إداري' : 'قرار تنظيمي عام')),
  ...
  const handleSaveDecree = (e?: React.FormEvent) => {
    ...
    if ((decreeForm.isAppointment || decreeForm.isThanksOrHonorary) && !decreeForm.appointeeName.trim()) {
      alert('يرجى إدخال اسم العضو المعني في القرار');
      return;
    }
      ...
      const updated = decreesList.map(d => {
        if (d.id === editingDecreeId) {
          return {
            ...d,
            decreeNumber: decreeForm.decreeNumber,
            decreeType: decreeForm.isThanksOrHonorary ? 'honorary' as const : (decreeForm.isAppointment ? 'appointment' as const : decreeForm.decreeType),
            isAppointment: decreeForm.isAppointment,
            isThanksOrHonorary: decreeForm.isThanksOrHonorary,
            includePhoto: decreeForm.includePhoto,
            thanksReason: decreeForm.thanksReason,
        ...
        <div className="space-y-8 animate-fadeIn">
          ...
          <div id="decree-form-anchor" className="bg-white rounded-3xl p-6 sm:p-8 border-2 border-[#d4af37] shadow-xl space-y-6">
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-b border-slate-100 pb-5">
              ...
              {/* Mode Toggle: Appointment vs Thanks vs General Administrative */}
              <div className="bg-slate-100 p-1.5 rounded-2xl flex flex-wrap items-center gap-1 text-xs font-bold">
                <button
                  type="button"
                  onClick={() => {
                    setDecreeForm(prev => ({
                    ...
                    handleAutoGenerateDecreeText(undefined, undefined, undefined, true, false);
                  }}
                  className={`px-3 py-2 rounded-xl transition-all cursor-pointer flex items-center gap-1.5 ${
                    decreeForm.isAppointment && !decreeForm.isThanksOrHonorary
                      ? 'bg-[#064e3b] text-white shadow'
                      : 'text-slate-600 hover:text-slate-900'
                  }`}
                >
                  <UserCheck className="w-4 h-4 text-[#d4af37]" />
                  <span>قرار تعيين وتكليف</span>
                </button>
                ...
                <button
                  ...
                  onClick={() => {
                    setDecreeForm(prev => ({
                      ...prev,
                      isAppointment: false,
                      isThanksOrHonorary: true,
                      decreeType: 'honorary'
                    }));
                    handleAutoGenerateDecreeText(undefined, undefined, undefined, false, true);
                  }}
                  className={`px-3 py-2 rounded-xl transition-all cursor-pointer flex items-center gap-1.5 ${
                    decreeForm.isThanksOrHonorary
                      ? 'bg-[#064e3b] text-[#d4af37] shadow'
                      : 'text-slate-600 hover:text-slate-900'
                  }`}
                >
                  <Award className="w-4 h-4 text-[#d4af37]" />
                  <span>قرار شكر وتقدير وعرفان</span>
                </button>
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    handleAutoGenerateDecreeText(undefined, undefined, undefined, false, false);
                  }}
                  className={`px-3 py-2 rounded-xl transition-all cursor-pointer flex items-center gap-1.5 ${
                    !decreeForm.isAppointment && !decreeForm.isThanksOrHonorary
                      ? 'bg-[#064e3b] text-white shadow'
            ...
            <form onSubmit={handleSaveDecree} className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <select
                    value={decreeForm.isThanksOrHonorary ? 'honorary' : decreeForm.decreeType}
                    onChange={(e) => {
                      const val = e.target.value as OfficialDecree['decreeType'];
                      const isAppt = val === 'appointment';
                      const isThanks = val === 'honorary';
                      setDecreeForm(prev => ({
                        ...prev,
                        decreeType: val,
                        isAppointment: isAppt,
                        isThanksOrHonorary: isThanks
                      }));
                      handleAutoGenerateDecreeText(undefined, undefined, undefined, isAppt, isThanks);
                    }}
                    className="w-full text-xs sm:text-sm p-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-[#064e3b] outline-none"
                  >
                    <option value="appointment">قرار تعيين وتكليف إداري</option>
                    <option value="honorary">قرار شكر وتقدير ووسام وفاء</option>
              ...
              {/* ======================================================== */}
              {/* APPOINTMENT & THANKS SPECIFIC SECTION (PHOTO & DETAILS) */}
              {/* ======================================================== */}
              {(decreeForm.isAppointment || decreeForm.isThanksOrHonorary) && (
                <div className="bg-gradient-to-br from-[#fcfbf7] to-[#f5f1e4] p-5 sm:p-6 rounded-2xl border-2 border-[#d4af37] space-y-5 shadow-sm">
                  <div className="flex flex-wrap items-center justify-between gap-2 border-b border-amber-200 pb-3">
                    <div className="flex items-center gap-2">
                      <div className="w-8 h-8 rounded-lg bg-[#064e3b] text-[#d4af37] flex items-center justify-center font-bold">
                        {decreeForm.isThanksOrHonorary ? <Award className="w-4 h-4" /> : <UserCheck className="w-4 h-4" />}
                      </div>
                      <div>
                        <h4 className="font-heritage text-base font-bold text-[#064e3b]">
                          {decreeForm.isThanksOrHonorary ? 'بيانات الشريف المُكرم وصورته والتكريم الهاشمي' : 'بيانات العضو المعيَّن وصورته الرسمية والتكليف الإداري'}
                        </h4>
                        <p className="text-xs text-slate-600">
                          {decreeForm.isThanksOrHonorary ? 'تخصيص الشكر والتقدير مع خيار إضافة الصورة أو الاسم فقط' : 'يتم وضع صورة العضو المعين داخل إطار مذهب فخم في أعلى القرار وتوليد الديباجة تلقائياً'}
                        </p>
                      </div>
                    </div>

                    {/* Photo vs Name-Only Mode Toggle */}
                    <div className="flex items-center gap-2 bg-white p-1 rounded-xl border border-amber-300 shadow-xs">
                      <span className="text-[11px] font-bold text-slate-600 px-1">طريقة الإبراز:</span>
                      <button
                        type="button"
                        onClick={() => setDecreeForm(prev => ({ ...prev, includePhoto: true }))}
                      ...
                      <button
                        type="button"
                        onClick={() => setDecreeForm(prev => ({ ...prev, includePhoto: false }))}
                        className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                          !decreeForm.includePhoto
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-12 gap-5 items-start">
                    ...
                    {/* Appointee Photo Box (Only if includePhoto is true) */}
                    {decreeForm.includePhoto ? (
                      ...
                      <div className="md:col-span-4 bg-emerald-50/70 p-4 rounded-2xl border border-emerald-200 text-center flex flex-col items-center justify-center space-y-2">
                        <Award className="w-10 h-10 text-[#d4af37]" />
                        <span className="text-xs font-bold text-[#064e3b]">صيغة الاسم فقط (بدون صورة)</span>
                        <p className="text-[11px] text-slate-600">
                          سيتم إبراز اسم الشريف المكرم بخط ملكي عريض في صدارة القرار دون إدراج صورة شخصية.
                        </p>
                      </div>
                    )}
                    ...
                    <div className="md:col-span-8 space-y-3">
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        ...
                        <div className="sm:col-span-2">
                          <label className="block text-xs font-bold text-slate-700 mb-1">
                            الاسم الكامل للشريف المعني *
                          </label>
                          <input
                            type="text"
                            required={decreeForm.isAppointment || decreeForm.isThanksOrHonorary}
